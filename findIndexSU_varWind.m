...
for encWindow = 5% 1:22
    ...
    for retWindow = 7% 1:22
        ...
        % 12 12 (cue to resp for enc and ret)
        % 6 12  (-1s cue to resp for enc; cue to resp for ret)
        ...
        numperm      = 10000;
        empNumFP2    = 0;
        savingpath   = ['X:\Luca\indexSUvisu\NEW!_varWindow_ENC_', num2str(timeWindowENC(1)), num2str(timeWindowENC(2)), num2str(lockingENC(1)), num2str(lockingENC(2)), '_RET_', num2str(timeWindowRET(1)), num2str(timeWindowRET(2)), num2str(lockingRET(1)), num2str(lockingRET(2)), '_DP_p',num2str(plvl),'_th',num2str(fixTH)];
        mkdir(savingpath)
        ...
        for ia = 1:size(allDat,1)
            ...
            load([allDat(ia).folder, filesep, allDat(ia).name], 'allSU');
            if isempty(allSU)
                continue
            end
...
%             cd(cdServ);
            ...
            for su = 1 : size(normENC,1)
                ...
                % new: save logfile info
                orgSU(suNum).bidsID     = bidsID;
                orgSU(suNum).sesh       = sesh;
                orgSU(suNum).wirename   = wireName;
                orgSU(suNum).bundlename = wireName(1:end-1);
                orgSU(suNum).su         = su;
                orgSU(suNum).encTrigger = encTrigger;
                orgSU(suNum).retTrigger = retTrigger;
                ...
                % DP
                dynTH                = prctile(permDP{suNum}(:),plvl); % OLD WAY. PROBLEM WITH MULT. COMP.
                %                 dynTH                = prctile(max(permDP{suNum},[],1),plvl); % NEW WAY. TAKING THE MAXIMUM VALUE
...
%                 orgSU(suNum).idxTrl     = ffTrls + ppTrls + fpTrls;
                ...
                if sum([ffTrls + ppTrls + fpTrls])
%                     orgSU(suNum).iu = 1; % this is a gray unit
                end
                
                % tests if a SU is a FP2 neuron
                if putIdx
%                     orgSU(suNum).iu = 2; % this is an index unit (FP2)
