...
load('X:\Luca\data\microLFP\sub-0009_S1_onlyMicroLFP_RAW_1000DS_SPKINT.mat', 'data'); % micro lfp data
load('allTrig.mat')
load('rpplRec.mat')
su = 260;
spks = allSU(su).spks;
...
trls   = floor(size(data.trial{1},2)/3000);
...
for trl = 1:trls % loop over trials
    dataTrl = data.trial{1}(:,npnts*(trl-1)+1:npnts*trl); %% trial data from continuous
    ...
    % segment spikestimes
%     idxSpksSeg{trlsEffect} = round(spks(spks >= npnts*(trlsEffect-1)+1 & spks <= npnts*trlsEffect) - npnts*(trlsEffect-1)+1);
      idxSpksSeg{trlsEffect} = round( spks(spks >= npnts*(trl-1)+1 & spks <= npnts*trl) - npnts*(trl-1)+1 );
