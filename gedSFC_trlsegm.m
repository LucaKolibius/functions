...
% Adapted for multiple trials by LDK 23.04.2020

%% the important stuff...
% we need to get this from simons simulation
function gedSFC_trlsegm(data, idxSpks, bidsID, sesh, su)
npad = 52;
npad2 = npad/2;
nchans = 8;
srate = 1000;
npnts  = 1001;
trls = size(data.segmDat,2);
...
for trl = 1:trls % loop over trials
    dataTrl = data.segmDat{1,trl}(:,:); %% trial data
    if any(isnan(dataTrl(:)))
        ...
        % segment spikestimes
    idxSpksSeg{trlsEffect} = round(idxSpks(idxSpks >= 1000*(trlsEffect-1)+1 & idxSpks <= 1000*trlsEffect) - 1000*(trlsEffect-1)+1);
...
spcovTrl = spcovTrl / trlsEffectSpks;
...
save(['X:\Luca\data\engram allocation lfp\trlsegm\', bidsID, '_', sesh, '_su', num2str(su)], 'jdmaps')
end
% %% plotting
% 
% figure(1), clf
