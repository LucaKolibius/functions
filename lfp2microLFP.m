...
LFPdir  = dir('X:\George\Analysis\Data Continuous\sub-*MACRO_RAW.mat');
...
%     %% FOR MICROS
%     labs    = data_macro.label; % all labels
%     noSpks  = cellfun(@(x) regexp(x, 'SPKS'), labs, 'un', 0); % all labels apart from the spikes
%     noTrig  = cellfun(@(x) regexp(x, 'TRIG'), labs, 'un', 0); % indexes the trigger
%     noHits  = cellfun(@(x) regexp(x, 'enc+'), labs, 'un', 0); % indexes the hits / miss info
%     selChan = cellfun(@isempty, noSpks);
...
%     selChan      = logical(selChan - noTrig - noHits);   
    ...
    cfg          = [];
    cfg.channel  = data_macro.label(selChan);       % all 8 channels
    data         = ft_selectdata(cfg, data_macro);  % select
...
%     end
    ...
    save(['X:\Luca\data\macroLFP\', bidsID, '_', sesh, '_onlyMacroLFP_RAW_1000DS.mat'], 'data')
