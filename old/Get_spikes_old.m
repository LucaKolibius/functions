function Get_spikes_old(input, varargin)
...
if isnumeric(input) || any(strcmp(input,'all'))  % cases for numeric or 'all' input
    ...
    for i = 1:length(dirnames)
        ...
        ext = lower(ext(2:end)); % makes it lower case and gets rid of the "."
        if any(strcmp(ext,se)) % is the extension (ext) among the allowed extension (se)?
            if strcmp(input,'all')
                if strcmp(ext,'mat')
                ...
                if ~isempty(aux) && ismember(str2num(aux{1}),input)
                    ...
                    filenames = [filenames {fname}]; % add fname to list of filenames
                end
            end
        end
    end
...
function get_spikes_single(filename, par_input)
    ...
    par = update_parameters(par,par_input,'relevant'); % updates parameters if we specified it when calling Get_spikes
    try 
        data_handler = readInData(par); % this updates 'sr' and adds 'channels' and 'ref'
    catch MExc
        warning(MExc.message);
        return
    end
    par = data_handler.par; % update parameter according to data_handler
    ...
    if data_handler.with_spikes            %data have some type of _spikes files
        ...
        for n = 1:data_handler.max_segments
            ...
            index = [index data_handler.index2ts(new_index)]; %new_index to ms % this was the original script
%            index = [ index new_index ]; % this doesnt work because it
%            starts at 0 for all 9 segments. an approach to fix this is to
%            look at size(x) which is exactly the number of samples in this
%            segment and then to add this to each subsequent segment
