...
%   POLARHISTOGRAM(THETA) plots an angluar histogram of THETA. Specify the
%   values in radians. POLARHISTOGRAM determines the bin edges using an 
%   automatic binning algorithm that returns uniform bins of a width that 
%   is chosen to cover the range of values in THETA and reveal the shape of 
%   the underlying distribution. 
%
%   POLARHISTOGRAM(THETA,M), where M is a scalar, uses M bins.
%
%   POLARHISTOGRAM(THETA,EDGES), where EDGES is a vector, specifies the 
%   edges of the bins.
%
...
%     
%   POLARHISTOGRAM('BinEdges', EDGES, 'BinCounts', COUNTS) where COUNTS is 
%   a vector of length equal to length(EDGES)-1, manually specifies the bin 
%   counts. POLARHISTOGRAM plots the counts and does not do any data binning.
%
...
%   than 65536 bins are needed, POLARHISTOGRAM uses wider bins instead.
%
...
%
%   POLARHISTOGRAM(PAX,...) plots into the polar axes PAX instead of the 
%   current axes.
%       
