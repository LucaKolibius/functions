...
for sesh = 1%:40
    ...
    subjID = strcat('P98_S', num2str(sesh));
...
% we have 30 SU
for su = 1:30
    rndidx(su,1) = randi(4); % random index
    x = -1;
    
    while any(x < 0) % this is the spike number vector for one SU, make sure we do not have negative numbers, by resampling
        x = round( normrnd( ff(rndidx(su)), sigm(rndidx(su)), 1, allTrials*2));
    end
    
    trialEnc(su+2, 1:allTrials) = num2cell(x(1:allTrials)); % the first 200 columns belong to encoding
    trialRet(su+2, 1:allTrials) = num2cell(x(allTrials+1:allTrials*2)); % the last 200 columns belong to retrieval
end
%% adding the WT and WC effect
indxFF = find(strcmp(trialEnc(1,:),'ff'));
meanNewFF = trialEnc(:,indxFF(1));

indxFP = find(strcmp(trialEnc(1,:),'fp'));
meanNewFP = trialEnc(:,indxFP(1));

indxPP = find(strcmp(trialEnc(1,:),'pp'));
meanNewPP = trialEnc(:,indxPP(1));

NEWtrialEnc = [];
% trial noise; depends on the FR
for ip = 1:size(indxFF,2)
    SUnoise = rndidx + randi([0 4],[30 1])./4;
    NEWtrialEnc(3:32,indxFF(ip)) = cell2mat(meanNewFF(3:end)) + SUnoise;
end

for ip = 1:size(indxFP,2)
    SUnoise = rndidx + randi([0 4],[30 1])./4;
    NEWtrialEnc(3:32,indxFP(ip)) = cell2mat(meanNewFP(3:end)) + SUnoise;
end

for ip = 1:size(indxPP,2)
    SUnoise = rndidx + randi([0 4],[30 1])./4;
    NEWtrialEnc(3:32,indxPP(ip)) = cell2mat(meanNewPP(3:end)) + SUnoise;
end

NEWtrialEnc = num2cell(NEWtrialEnc);
NEWtrialEnc(1:2,:) = [];
NEWtrialEnc = [trialEnc(1:2,:); NEWtrialEnc];



trialEnc = NEWtrialEnc;
trialRet = NEWtrialEnc;
%%
% make into a table
